const BACKEND_URL = ""; // same-origin — the site and API share one domain

// --- localStorage helpers (mirror the extension's chrome.storage.local shape) ---
...
// --- History (server-side now — a website has no browser-local storage
// that follows a user across devices the way the extension's did) ---
...
async function renameHistoryEntry(id, newName) {
  const { token } = await getStoredAuth();
...
function startRenamingEntry(entry, row, nameEl) {
  ...
  const commit = async () => {
    ...
    const history = await renameHistoryEntry(entry.id, newName);
...
function renderHistory(history) {
  ...
  for (const entry of history) {
    ...
    meta.textContent = `${entry.grade || ""} · ${new Date(entry.created_at).toLocaleString()}`;
...
async function getStoredAuth() {
  const { authToken, authUser } = await storageGet(["authToken", "authUser"]);
  return { token: authToken || null, user: authUser || null };
}

async function setStoredAuth(token, user) {
  await storageSet({ authToken: token, authUser: user });
}
...
async function getRememberedEmail() {
  const { rememberedEmail } = await storageGet("rememberedEmail");
  return rememberedEmail || "";
}

async function setRememberedEmail(email) {
  if (email) {
    await storageSet({ rememberedEmail: email });
  } else {
    await storageRemove("rememberedEmail");
  }
}
...
async function purchasePlan(planId, buyBtn) {
  ...
  try {
    ...
    // Same-tab redirect — standard website checkout UX (unlike the
    // extension, which has to use a new tab since it can't navigate its
    // own side panel to an external domain).
...
// Refresh the balance whenever the tab regains focus — useful if the user
// has the app open in two tabs, or comes back after checking out.
